...
void maxStack<T, max_size>::push(T ele) {
    ...
    // checking isEmpty()
        ...
        arr[length++] = ele;
        cout<<"Pushed the first element "<<ele<<endl;
      }
      ...
      // if not empty
        ...
        // push ele on the top
        arr[length++] = ele;
        cout<<"Pushed another ele "<<ele<<endl<<arr[length-1]<<endl;  
      }

      // if pushed ele is smaller than the top
      else if (ele < top()){
        cout<<"Received input = "<<ele<<endl;

        // current top is assigned to temp
        // could make this a template
        int temp = arr[length-1];
        cout<<"Assigned "<<arr[length-1]<<" to temp "<<temp<<endl;

        // remove top
        pop();
        cout<<"Popped the top ele "<<arr[length]<<endl;

        // put ele on top
        arr[length++] = ele;
        cout<<"Push ele "<<ele<<" on top "<<endl;

        // put temp on top of ele as it is smaller than the previous top
        arr[length++] = temp;
        cout<<"Push temp "<<temp<<" on top of "<<ele<<endl;
      }
    }
